"""
Derived Metrics Model — Pre-calculated financial metrics (upserted by recalculation engine).
"""
...
class DerivedMetrics(Base):
    ...
    cash_in_hand = Column(Numeric(15, 2), nullable=True)

    # Timestamps
    updated_at = Column(DateTime(timezone=True), server_default=func.now(), onupdate=func.now())

    # Relationship
    user = relationship("User", back_populates="derived_metrics")
